...
GUARDRAILS:
- The VISUAL is the hero and does the storytelling; the headline is short, sleek, and supporting. Never build a text poster, a caption bar, or a wall of text - keep on-image text minimal and let the visual carry the meaning.
- Make this image visually UNIQUE - a distinct subject, composition, and treatment drawn from THIS post - so it never looks like a template or a repeat of another post's image.
- Choose the style purely from THIS post's content and this person's profile (role, positioning, preferences), NOT their industry, so it genuinely varies from other posts.
